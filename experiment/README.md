...
- Queue migration v4: replayed a manifest-anchored anonymized archive trace using timestamps and role labels only; protocol in `docs/queue-migration-v4-protocol.md`, results in `docs/queue-migration-v4-results.md` and receipt in `artifacts/queue_migration_v4.json`. A second corpus trace was replayed without retuning; results in `docs/queue-migration-v4-trace2-results.md` and receipt in `artifacts/queue_migration_v4_trace2.json`.
- Google Borg BigQuery v1: remote aggregate over public `clusterdata_2019_a.collection_events`; raw rows remain in BigQuery. Protocol in `docs/google-borg-bigquery-v1-protocol.md`, results in `docs/google-borg-bigquery-v1-results.md`, receipt in `artifacts/google_borg_bigquery_v1.json`, and query/result metadata under `sources/google-borg/`.
- Google Borg BigQuery v2: remote lifecycle-envelope aggregate over public `clusterdata_2019_a.instance_events`; raw rows remain in BigQuery. Protocol in `docs/google-borg-bigquery-v2-protocol.md`, results in `docs/google-borg-bigquery-v2-results.md`, receipt in `artifacts/google_borg_bigquery_v2.json`.
- Google Borg BigQuery v3: remote attempt-level reconstruction for the first 24-hour window; right-censored attempts are explicit. Protocol in `docs/google-borg-bigquery-v3-protocol.md`, results in `docs/google-borg-bigquery-v3-results.md`, receipt in `artifacts/google_borg_bigquery_v3.json`.
- Google Borg BigQuery v4: remote five-minute pressure join of arrivals, admissions, terminal outcomes, and attempt summaries; startup-boundary bucket reported separately. Protocol in `docs/google-borg-bigquery-v4-protocol.md`, results in `docs/google-borg-bigquery-v4-results.md`, receipt in `artifacts/google_borg_bigquery_v4.json`.
- Google Borg BigQuery v5: remote join against machine capacity and task-usage proxies; correlations are descriptive and startup bucket is excluded. Protocol in `docs/google-borg-bigquery-v5-protocol.md`, results in `docs/google-borg-bigquery-v5-results.md`, receipt in `artifacts/google_borg_bigquery_v5.json`.
- Google Borg BigQuery v6: frozen v3-v5 replication on independent public cell `b`; cross-cell heterogeneity is explicit. Protocol in `docs/google-borg-bigquery-v6-protocol.md`, results in `docs/google-borg-bigquery-v6-results.md`, receipt in `artifacts/google_borg_bigquery_v6.json`.
- Google Borg BigQuery v7: cell-b attempt outcomes stratified by priority, scheduling class, collection type, and coarse resource-request bins; exact conservation audit passes. Protocol in `docs/google-borg-bigquery-v7-protocol.md`, results in `docs/google-borg-bigquery-v7-results.md`, receipt in `artifacts/google_borg_bigquery_v7.json`.
- Google Borg BigQuery v8: paired v7 scheduler stratification on cells `a` and `b`; independent conservation audits pass and cell-specific composition is explicit. Protocol in `docs/google-borg-bigquery-v8-protocol.md`, results in `docs/google-borg-bigquery-v8-results.md`, receipt in `artifacts/google_borg_bigquery_v8.json`.
